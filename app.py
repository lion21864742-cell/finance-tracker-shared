...
# ══════════════════════════════════════════════════════
    ...
    with tab2:
        ...
        with st.form("add_trade_form", clear_on_submit=True):
            ...
            invest_acc_options = list(st.session_state.my_assets.keys())
            default_invest_idx = invest_acc_options.index("投資帳戶 📈") if "投資帳戶 📈" in invest_acc_options else 0
            auto_sync_acc = st.selectbox(
                "💰 自動計入資產帳戶（賣出+/買入−）", invest_acc_options,
                index=default_invest_idx if invest_acc_options else 0,
                key="tr_auto_sync_acc", help="勾選下方選項後，賣出所得會自動加入此帳戶；買入成本會自動扣除"
            ) if invest_acc_options else None
            do_auto_sync = st.checkbox("✅ 啟用自動同步（賣出所得＋ / 買入成本－）", value=True, key="tr_do_auto_sync")
            ...
            if st.form_submit_button("✅ 確認記錄", use_container_width=True):
                if tr_name.strip() and tr_qty > 0 and tr_price > 0:
                    ...
                    sync_msg = ""
                    if do_auto_sync and auto_sync_acc:
                        net_hkd = net * usd_to_hkd if tc == "USD" else net
                        current_bal = float(st.session_state.my_assets.get(auto_sync_acc, 0))
                        if is_buy:
                            st.session_state.my_assets[auto_sync_acc] = current_bal - net_hkd
                            sync_msg = f"　|　💸 已從「{auto_sync_acc}」扣除 HK${net_hkd:,.2f}"
                        else:
                            st.session_state.my_assets[auto_sync_acc] = current_bal + net_hkd
                            sync_msg = f"　|　💰 已將 HK${net_hkd:,.2f} 計入「{auto_sync_acc}」"

                    save_now(); refresh_holdings()
                    st.success(f"✅ {'買入' if is_buy else '賣出'} {tr_name.strip()} × {tr_qty} @ {tr_price}{sync_msg}")
                    st.rerun()
                else:
